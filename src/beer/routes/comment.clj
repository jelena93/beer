...
(defresource add-comment [{:keys [params session]}]
  ...
  :exists? (fn [_] (not (empty? (db/find-beer (select-keys (rename-keys params {:beer :id}) [:id])))))
  ...
  :handle-created (fn [_] (render-map-generic (db/find-comment (select-keys params [:beer]))))
...
(defresource delete-comment [{:keys [params session]}]
  ...
  :exists? (fn [_] (not (empty? (db/find-comment (select-keys params [:id])))))
  ...
  :handle-ok (fn [_] (json/write-str (count (db/find-comment (select-keys params [:beer])))))
  :available-media-types ["application/json"])
